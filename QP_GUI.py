...
def pub_qp_data():
    ...
    if operator.get() == 2:
        ...
        for token in inputText:
            if len(inputText) < 2:
                messagebox.showinfo(title="Hey!", message="Please enter at least 2 words to apply operator.")
                break
            if len(token) <= 3:
...
label = Label(window, text="Coventry University Search Engine", bg="#263543", fg="white", font="'Arial' 24 bold")
label.config(anchor=CENTER)
label.pack()
apply = Label(window, text='SEARCH :')
count = Label(window, text='TOTAL :')
...
search = Button(window, text='SEARCH', bg="#D3D3D3", fg="#263543", font="'MONOSPACE' 10", command=pub_qp_data)
